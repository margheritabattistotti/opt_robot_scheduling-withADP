...
# take [:3] for MINI instance, [:4] for MEDIUM, all for LARGE
...
# Thorizon = 250  # (s) for LARGE instance
                    ...
                    for action_type in Q.keys():
                        ...
                        for k in Q[action_type].keys():
                            ...
                            if action_type == 'throw':
                                ...
                                failed_s = s.copy()
                                failed_s[0] += tthrow
                                failed_s[2 + (T+1) * k[0]] -= 1  # Throwing failed: object lost
                                v = v * p_success + (1 - p_success) * (discount_factor * V[tuple(failed_s)])
                            V_temp.append(v)
            ...
            for k in Q[action_type].keys():
                ...
                if candidate_value > max_value:
                    ...
                    if action_type == 'throw' and \
                        ...
                        best_action = action_type + ' ' + str(k) + '(FAILED)'
                    else:
