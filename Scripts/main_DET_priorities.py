...
placing_nodes = nodes[5:7]  # for MEDIUM and TEST instance
...
def updated_state_space(current_state, empty_tray, tot_obj_dict, maxObj):
    # Generates all possible final states in terms of picked and placed object types, without accounting for
    # time and node position. Tailored for 5 object types and 2 trays.
    # INPUT
    # current_state: current state of the system
    # empty_tray: the just emptied tray index to be substituted (tray0=0, tray1=1)
    # tot_obj_dict: {object: total amount to be placed during mission}, output of objects4mission_dict(mission)
    # maxObj: maximum number of objects to be carried at a time by the robot
    # OUTPUT
    # all_final_state: list of all possible state configurations

    # 1) Definition of all possible states such that picked[i]>=placed[i] for all objects i

    # MEDIUM INSTANCE
    cut_state = current_state[2:17]

    if empty_tray == 1:
        all_states = [[a1, a2, a3, b1, b2, b3, c1, c2, c3, d1, d2, d3, e1, e2, e3]
                      for a1 in range(cut_state[0], list(tot_obj_dict.values())[0] + 1)
                      for a2 in range(cut_state[1], list(list(mission.values())[0].values())[0] + 1)
                      for a3 in range(0, list(list(mission.values())[1].values())[0] + 1)
                      for b1 in range(cut_state[3], list(tot_obj_dict.values())[1] + 1)
                      for b2 in range(cut_state[4], list(list(mission.values())[0].values())[1] + 1)
                      for b3 in range(0, list(list(mission.values())[1].values())[1] + 1)
                      for c1 in range(cut_state[6], list(tot_obj_dict.values())[2] + 1)
                      for c2 in range(cut_state[7], list(list(mission.values())[0].values())[2] + 1)
                      for c3 in range(0, list(list(mission.values())[1].values())[2] + 1)
                      for d1 in range(cut_state[9], list(tot_obj_dict.values())[3] + 1)
                      for d2 in range(cut_state[10], list(list(mission.values())[0].values())[3] + 1)
                      for d3 in range(0, list(list(mission.values())[1].values())[3] + 1)
                      for e1 in range(cut_state[12], list(tot_obj_dict.values())[4] + 1)
                      for e2 in range(cut_state[13], list(list(mission.values())[0].values())[4] + 1)
                      for e3 in range(0, list(list(mission.values())[1].values())[4] + 1)]
    else:
        all_states = [[a1, a2, a3, b1, b2, b3, c1, c2, c3, d1, d2, d3, e1, e2, e3]
                      for a1 in range(cut_state[0], list(tot_obj_dict.values())[0] + 1)
                      for a2 in range(0, list(list(mission.values())[0].values())[0] + 1)
                      for a3 in range(cut_state[2], list(list(mission.values())[1].values())[0] + 1)
                      for b1 in range(cut_state[3], list(tot_obj_dict.values())[1] + 1)
                      for b2 in range(0, list(list(mission.values())[0].values())[1] + 1)
                      for b3 in range(cut_state[5], list(list(mission.values())[1].values())[1] + 1)
                      for c1 in range(cut_state[6], list(tot_obj_dict.values())[2] + 1)
                      for c2 in range(0, list(list(mission.values())[0].values())[2] + 1)
                      for c3 in range(cut_state[8], list(list(mission.values())[1].values())[2] + 1)
                      for d1 in range(cut_state[9], list(tot_obj_dict.values())[3] + 1)
                      for d2 in range(0, list(list(mission.values())[0].values())[3] + 1)
                      for d3 in range(cut_state[11], list(list(mission.values())[1].values())[3] + 1)
                      for e1 in range(cut_state[12], list(tot_obj_dict.values())[4] + 1)
                      for e2 in range(0, list(list(mission.values())[0].values())[4] + 1)
                      for e3 in range(cut_state[14], list(list(mission.values())[1].values())[4] + 1)]
...
def new_state_picking(a, s):
    ...
    new_s = s.copy()
    new_s[0] += tpick
    new_s[2 + (T+1) * a] += 1  # assuming that action a contains the index of the object to pick
    return new_s
...
def new_state_placing(a, s):
    ...
    # assuming that action a contains the index of the object to place and the index of the tray
    return new_s
    ...
    while Q is not None:  # Time horizon not reached or mission not completed yet
        ...
        for action_type in Q.keys():
            ...
            for k in Q[action_type].keys():
                if action_type == 'place':
                    # Craffa3) + Sergio) no threshold
                    immediate_contribution = rewards[action_type] * (1 - (s[0] / trays_delivery_times[k[1]])) - \
                                             0.001 * (s[0] - trays_entering_times[k[1]]) \
                                             -100 / trays_delivery_times[k[1]] * (trays_delivery_times[k[1]] < s[0])
        ...
        for t in range(T):
            if check_full_tray[t] == obj4trays_dict[trays[t]] and len(orders) != 0:
                    ...
                    states_list = updated_state_space(s, t, obj4mission_dict, maxPortableObjs)
