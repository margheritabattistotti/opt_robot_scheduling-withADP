...
orders = [[{"objectA": 3, "objectB": 2, "objectC": 1, "objectD": 2, "objectE": 1}, 370],
          [{"objectA": 2, "objectB": 2, "objectC": 4, "objectD": 3, "objectE": 0}, 550],
          [{"objectA": 1, "objectB": 2, "objectC": 3, "objectD": 2, "objectE": 3}, 230]]
rg_move_stream = random.Random(2404)
rg_throw_stream = random.Random(610)
...
def new_state_throwing(a, s):
    ...
    new_s = s.copy()
    new_s[0] += tthrow
    new_s[3 + (T+1) * a[0] + a[1]] += 1  # assuming that action a contains the index of the object to place
    # if there is failure should we remove the object from the picked ones? YES look in forward_pass()
    return new_s
